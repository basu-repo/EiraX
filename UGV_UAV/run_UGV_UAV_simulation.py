...
from UGV_Standalone.baseline.data_logging.run_dataset import RunDataset
from UGV_Standalone.baseline.launchers import commands
...
from UGV_Standalone.baseline.mission.world_poses import (
    named_pose,
...
CONFIG_FILE = PROJECT_ROOT / "UGV_Standalone/baseline/config/baseline.yaml"
...
def main() -> int:
    ...
    if args.uav_follow:
        ...
        uav_pad = build_cooperative_world(world_file, cooperative_world)
    ...
    if args.uav_follow:
        ...
        env["PYTHONPATH"] = ":".join(
            [
                ...
                str(PROJECT_ROOT),
                str(PROJECT_ROOT / "UGV_Standalone"),
                env.get("PYTHONPATH", ""),
            ]
        ).rstrip(":")
    ...
    try:
        ...
        manager.start(
            ...
            commands.local_localization(
                PROJECT_ROOT / "UGV_Standalone/baseline/config/local_ekf_params.yaml"
            ),
        )
        manager.start(
            "localization",
            commands.localization(
                PROJECT_ROOT / "UGV_Standalone/baseline/config/ekf_params.yaml"
            ),
        )
        manager.start(
            "navsat_transform",
            commands.navsat_transform(
                PROJECT_ROOT / "UGV_Standalone/baseline/config/navsat_params.yaml"
            ),
        )
        ...
        if args.view_3d_slam and not args.headless:
            ...
            manager.start(
                ...
                commands.slam3d_viewer(
                    PROJECT_ROOT
                    / "UGV_Standalone/baseline/config/ugv_rtabmap_gui.ini"
                ),
            )
        ...
        if args.uav_follow:
            ...
            while not uav_ready_file.exists() and time.monotonic() < ready_deadline:
                if uav_follower.process.poll() is not None:
                    print("[FAILED] UAV follower stopped before reaching its overhead position.")
                    return 3
                time.sleep(0.5)
            if not uav_ready_file.exists():
                print("[FAILED] UAV did not reach its overhead follow position.")
                return 3
            print("[UAV READY] Takeoff complete; waiting for the first aerial survey.")
            ...
            for leg_index, target_name in enumerate(mission_targets, start=1):
                ...
                print(
                    f"[SURVEY] UAV is mapping the corridor to {target_name}; "
            ...
            uav_stop_file.touch()
            if mission_code == 0:
                print("[UAV LANDING] Goal reached; landing near the UGV at the goal.")
            else:
                print(
                    "[UAV LANDING] UGV mission stopped early; "
                    "landing near its final position."
                )
