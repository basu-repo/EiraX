...
from uav.failover_protocol import read_state
...
class HuskyGnss(Node):
    def __init__(self, uav_id: str, odom_topic: str, direct_follow: bool) -> None:
        super().__init__(f"{uav_id}_husky_gnss_follower")
        self.uav_id = uav_id
        self.odom_frame = f"{uav_id}_px4_odom" if uav_id != "uav" else "uav_px4_odom"
        self.base_frame = f"{uav_id}_base_link" if uav_id != "uav" else "uav_base_link"
        ...
        self.px4_odom = self.create_publisher(Odometry, odom_topic, 20)
        ...
        self.planner = None if direct_follow else ActionClient(
...
def main() -> int:
    ...
    parser.add_argument("--uav-id", default="uav")
    parser.add_argument("--odom-topic", default="/uav/px4_odom")
    parser.add_argument("--source-system", type=int, default=251)
    parser.add_argument("--failover-id", default="uav0")
    parser.add_argument("--swarm-state-file", type=Path)
    parser.add_argument("--preserve-coordination", action="store_true")
    parser.add_argument("--formation-north", type=float, default=0.0)
    parser.add_argument("--formation-east", type=float, default=0.0)
    parser.add_argument(
        "--landing-delay-sec", type=float, default=0.0,
        help="Delay touchdown after reaching the launch bay for sequential landing",
    )
    parser.add_argument(
        "--direct-follow",
        action="store_true",
        help="Use bounded direct formation steps without sharing UAV 0's Nav2 planner",
    )
    ...
    if args.coordination_dir is not None and not args.preserve_coordination:
    ...
    rclpy.init()
    node = HuskyGnss(args.uav_id, args.odom_topic, args.direct_follow)
    connection = mavutil.mavlink_connection(
        f"udpin:0.0.0.0:{args.port}", source_system=args.source_system
    )
    ...
    try:
        ...
        last_plan = -10.0
        ready = False
        vertical_takeoff_complete = False
        latest_local = None
        leg_index = 0
        ...
        def planned_step(goal_north: float, goal_east: float, current_north: float, current_east: float):
            ...
            if args.direct_follow:
                delta_north = goal_north - current_north
                delta_east = goal_east - current_east
                distance = math.hypot(delta_north, delta_east)
                if distance <= args.lookahead:
                    return goal_north, goal_east
                return (
                    current_north + delta_north * args.lookahead / distance,
                    current_east + delta_east * args.lookahead / distance,
                )
        ...
        with args.output.open("w", newline="", encoding="utf-8") as stream:
            ...
            while not args.stop_file.exists():
                ...
                role = "scout" if survey_targets else "follower"
                if args.swarm_state_file is not None and args.swarm_state_file.exists():
                    role = read_state(args.swarm_state_file).role_of(args.failover_id)
                if role in {"returning", "landed"}:
                    print(f"[FAILOVER] {args.failover_id} classified permanent; returning to its bay")
                    break
                # Every aircraft tracks completed legs, including reserves.
                # A newly elected scout therefore starts at the current
                # corridor instead of flying back to resurvey old waypoints.
                if survey_targets and leg_index < len(survey_targets):
                    complete_file = (
                ...
                if survey_targets and phase in ("survey", "escort") and role == "scout":
                ...
                if role in {"disconnected", "recovering"}:
                    # A transient outage never sends the aircraft home. Hold
                    # using PX4-local control until the link is restored.
                    command_north, command_east = message.x, message.y

                command_north += args.formation_north
                command_east += args.formation_east
                ...
                if not node.link_active or separation >= args.link_warning_range:
                    ...
                    command_north = north + args.formation_north
                    command_east = east + args.formation_east

                altitude_ready = abs(message.z - cruise_down) <= 1.0
                if not vertical_takeoff_complete:
                    # Clear the deck and nearby aircraft vertically before
                    # accepting any horizontal formation/survey command.
                    active_target[:] = [0.0, 0.0, cruise_down]
                    if altitude_ready:
                        vertical_takeoff_complete = True
                        print(
                            f"[UAV TAKEOFF] {args.uav_id} vertical climb complete; "
                            "translating to its formation slot"
                        )
                        last_plan = -10.0
                elif elapsed - last_plan >= 1.0:
                    step = planned_step(
                        command_north, command_east, message.x, message.y
                    )
                    if step is None:
                ...
                overhead_error = math.sqrt(
                    (message.x - command_north) ** 2
                    + (message.y - command_east) ** 2
                ...
                stream.flush()
                takeoff_ready = vertical_takeoff_complete and altitude_ready
                ...
                if elapsed - last_report >= 5.0:
                    ...
                    print(f"[SWARM ROLE] id={args.failover_id} role={role}")
                    last_report = elapsed

            # Each PX4 local origin is its own marked bay. Return to that known
            # flat surface rather than selecting unverified ground near the UGV.
            if latest_local is None:
                raise RuntimeError("No UAV local position available for landing")
            landing_north = 0.0
            landing_east = 0.0
            active_target[:] = [landing_north, landing_east, cruise_down]
            print(f"[UAV RETURN] {args.uav_id} returning above its assigned launch bay")
            approach_deadline = time.monotonic() + 300.0
            while time.monotonic() < approach_deadline:
                ...
                raise TimeoutError("UAV did not return above its launch bay")
            goal_vicinity_error = math.hypot(
                latest_local.x - north,
                latest_local.y - east,
            )
            if args.landing_delay_sec > 0.0:
                print(
                    f"[UAV LANDING QUEUE] {args.uav_id} holding over its bay for "
                    f"{args.landing_delay_sec:.0f} s"
                )
                time.sleep(args.landing_delay_sec)
            # Stay in offboard position control for the descent. Switching to
            # MAV_CMD_NAV_LAND releases the explicit bay X/Y target and allowed
            # the scout to drift 7.4 m off the deck in the full-route run.
            active_target[:] = [landing_north, landing_east, 0.0]
            print(
                f"[UAV LANDING] {args.uav_id} holding bay X/Y and descending "
                "vertically in offboard mode"
            )
            landing_deadline = time.monotonic() + 300.0
            disarm_requested = False
            landing_position_verified = False
            while time.monotonic() < landing_deadline:
                ...
                if message.get_type() == "LOCAL_POSITION_NED":
                    ...
                    stream.flush()
                    height_ready = abs(float(message.z)) <= 0.45
                    speed_ready = abs(float(message.vz)) <= 0.60
                    landing_position_verified = (
                        landing_horizontal_error <= 0.60 and height_ready
                    )
                    if (
                        not disarm_requested
                        and landing_position_verified
                        and speed_ready
                    ):
                        print(
                            f"[UAV TOUCHDOWN VERIFIED] bay error="
                            f"{landing_horizontal_error:.2f} m, local height="
                            f"{abs(float(message.z)):.2f} m; requesting disarm"
                        )
                        try:
                            command_and_wait(
                                connection,
                    ...
                    if landing_position_verified:
                        print(
                            f"[UAV LANDED VERIFIED] {args.uav_id} disarmed "
                            "within 0.60 m of its launch-bay centre"
                        )
                        return 0
                    print(
                        f"[UAV LANDING FAILED] {args.uav_id} disarmed without "
                        "a verified launch-bay position"
                    )
                    return 2
            return 2
    finally:
